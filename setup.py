...
def check_google_client():
    ...
    say()
    say()
    say("  ⚠️⚠️ 还有关键的第 7 步，漏了同步会在 7 天后自动死掉：")
    say("     回到 Audience 页面 → Publishing status 下点 【PUBLISH APP】 → 确认")
    say("     （按钮点不动的话，先去 Branding 页面把 App name 填上并 Save）")
    say("     原因：Testing 状态签发的授权，Google 规定满 7 天必失效。发布不需要过审。")
    say()
